...
class ProductsRepository implements IProductsRepository {
  ...
  public async findAllById(products: IFindProducts[]): Promise<Product[]> {
    ...
    const orderList = await this.ormRepository.find({ id: In(productsIdList) });

    if (productsIdList.length !== orderList.length) {
  ...
  public async updateQuantity(
    ...
    const newProducts = productsData.map(productData => {
      ...
      const newQtdProduct = productData;

      newQtdProduct.quantity -= productFind.quantity;

      return newQtdProduct;
    });
